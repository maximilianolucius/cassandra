...
INHIBIT_STATES = [-5, -3, 3, 4, 5, 6, 7, 9]       # Inhibit state list.
...
class ONCassandraTickProcessor:
    ...
    def save_fibo_levels_to_csv(self):
        ...
        fibo_data = {
            'price_name': [
                'base_price', 'top_price', 'fibo_n200', 'fibo_n161',
                'fibo_n138', 'fibo_base', 'fibo_38', 'fibo_50',
                'fibo_61', 'fibo_top', 'fibo_138', 'fibo_150',
                'fibo_161', 'fibo_200'
            ],
            'price_values': [
                self.nbi,  # base_price
                self.nti,  # top_price
                self.fibo_n200, self.fibo_n161, self.fibo_n138, self.fibo_base,
                self.fibo_38, self.fibo_50, self.fibo_61, self.fibo_top,
                self.fibo_138, self.fibo_150, self.fibo_161, self.fibo_200
            ]
        }
    ...
    def _calculate_fibonacci_levels(self):
        ...
        # Negative Fibonacci extensions
        self.fibo_n200 = self.fibo_base - (self.fibo_top - self.fibo_base)  # -200% extension
        self.fibo_n161 = self.fibo_base - 0.618 * size_  # -161.8% extension
        self.fibo_n138 = self.fibo_base - 0.382 * size_  # -138.2% extension
    ...
    def _compute_state(self, ref_price: float, prev_ref_price: float, Z: int, verbose=False):
        """
        Update the state (Z) based on price crossing Fibonacci levels in both directions.
        Handles both bullish (upward) and bearish (downward) transitions across extended Fibonacci levels.
        """
        # Bearish transitions (price crossing below levels)
        if ref_price < self.fibo_n200 and prev_ref_price >= self.fibo_n200:
            Z = -5
        elif ref_price < self.fibo_n161 and prev_ref_price >= self.fibo_n161:
            Z = -3
        elif ref_price < self.fibo_n138 and prev_ref_price >= self.fibo_n138:
            Z = -1
        ...
        # Bullish transitions (price crossing above levels)
        if ref_price > self.fibo_200 and prev_ref_price <= self.fibo_200:
            ...
            Z = -2
        elif ref_price > self.fibo_n200 and prev_ref_price <= self.fibo_n200:
            Z = -4
...
# ------------------------------------------------------------------------------
MT4_FILES_DIR = r'C:\Users\maxim\AppData\Roaming\MetaQuotes\Terminal\CBF2F6AE4D36F7FF1F027A9DADBE3E5A\MQL4\Files'
